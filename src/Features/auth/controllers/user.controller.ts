...
export class UserController {
    ...
    try {
      ...
      if (!Object.keys(updates).some((item) => allowedFields.includes(item))) {
        return res.status(404).json({
          ...
          data: ["firstname", "lastname", "country", "phoneNumber"],
        });
      }
  ...
  static async addBookmark(req: Request, res: Response) {
    try {
      ...
      return res
        .status(201)
        .json({message: "Property bookmarked successfully", data: newBookmark});
    } catch (error) {
  ...
  static async addSavedForLater(req: Request, res: Response) {
    try {
      ...
      const existingSavedForLater = await SavedForLater.findOne({
        user: id,
        property: propertyId,
      });
      ...
      const newSavedForLater = new SavedForLater({
        user: id,
        property: propertyId,
      });
      ...
      return res
        .status(201)
        .json({
          message: "Property saved for later successfully",
          data: newSavedForLater,
        });
    } catch (error) {
      return res.status(500).json({error: "Internal server error"});
    }
  ...
  static async removeSavedForLater(req: Request, res: Response) {
    try {
      ...
      return res
        .status(200)
        .json({message: "SavedForLater removed successfully"});
    } catch (error) {
      return res.status(500).json({error: "Internal server error"});
    }
  }
